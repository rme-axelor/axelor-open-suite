...
public class SaleOrderWorkflowServiceImpl implements SaleOrderWorkflowService {
	...
	@Inject
	public SaleOrderWorkflowServiceImpl(SequenceService sequenceService, PartnerRepository partnerRepo,
	...
	public String getSequence(Company company) throws AxelorException {
		...
		if (seq == null) {
			throw new AxelorException(company, TraceBackRepository.CATEGORY_CONFIGURATION_ERROR,
					I18n.get(IExceptionMessage.SALES_ORDER_1), company.getName());
		}
		return seq;
	}
	...
	public void finalizeQuotation(SaleOrder saleOrder) throws AxelorException {
		...
		Blocking blocking = Beans.get(BlockingService.class).getBlocking(partner, saleOrder.getCompany(),
				BlockingRepository.SALE_BLOCKING);
	...
	public void saveSaleOrderPDFAsAttachment(SaleOrder saleOrder) throws AxelorException {
		...
		if (saleOrder.getPrintingSettings() == null) {
			if (saleOrder.getCompany().getPrintingSettings() != null) {
				...
				throw new AxelorException(TraceBackRepository.CATEGORY_MISSING_FIELD,
						String.format(I18n.get(IExceptionMessage.SALE_ORDER_MISSING_PRINTING_SETTINGS),
		...
		ReportFactory
				.createReport("saleSaleOrder", saleOrder.getCompany(), IReport.SALES_ORDER,
				...
				.addParam("FooterHeight", saleOrder.getPrintingSettings().getPdfFooterHeight()).toAttach(saleOrder)
				.generate().getFileLink();

		// String relatedModel =
		// generalService.getPersistentClass(saleOrder).getCanonicalName();
		// required ?

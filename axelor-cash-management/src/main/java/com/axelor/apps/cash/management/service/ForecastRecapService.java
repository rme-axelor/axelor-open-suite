...
public class ForecastRecapService {
	...
	@Inject
	public ForecastRecapService(AppBaseService appBaseService, ForecastRepository forecastRepo,
			ForecastRecapLineRepository forecastRecapLineRepo, CurrencyService currencyService,
			ForecastRecapLineTypeRepository forecastRecapLineTypeRepo, ForecastRecapRepository forecastRecapRepo,
			OpportunityRepository opportunityRepo, InvoiceRepository invoiceRepo, EmployeeRepository employeeRepo,
	...
	public void populateWithOpportunities(ForecastRecap forecastRecap) throws AxelorException {
		...
		ForecastRecapLineType opportunityForecastRecapLineType = this
				.getForecastRecapLineType(ForecastRecapLineTypeRepository.ELEMENT_OPPORTUNITY);
		List<Integer> statusList = StringTool.getIntegerList(opportunityForecastRecapLineType.getStatusSelect());
		opportunityList = opportunityRepo.all().filter(
				"self.company = ?1" + (forecastRecap.getBankDetails() != null ? " AND self.bankDetails = ?2" : "")
				...
				forecastRecap.getToDate(), statusList).fetch();
		for (Opportunity opportunity : opportunityList) {
			...
			if (forecastRecap.getOpportunitiesTypeSelect() == ForecastRecapRepository.OPPORTUNITY_TYPE_BASE) {
				amountCompanyCurr = currencyService
						.getAmountCurrencyConvertedAtDate(opportunity.getCurrency(),
								opportunity.getCompany().getCurrency(),
								opportunity.getAmount().multiply(opportunity.getProbability())
				...
				amountCompanyCurr = currencyService
						.getAmountCurrencyConvertedAtDate(opportunity.getCurrency(),
								opportunity.getCompany().getCurrency(),
								opportunity.getBestCase().multiply(opportunity.getProbability())
										.divide(new BigDecimal(100), 2, RoundingMode.HALF_UP),
								today)
						.setScale(2, RoundingMode.HALF_UP);
				...
				amountCompanyCurr = currencyService
						.getAmountCurrencyConvertedAtDate(opportunity.getCurrency(),
								opportunity.getCompany().getCurrency(),
								opportunity.getWorstCase().multiply(opportunity.getProbability())
										.divide(new BigDecimal(100), 2, RoundingMode.HALF_UP),
								today)
						.setScale(2, RoundingMode.HALF_UP);
			}
			this.createForecastRecapLine(opportunity.getExpectedCloseDate(),
					opportunityForecastRecapLineType.getTypeSelect(), amountCompanyCurr, Opportunity.class.getName(),
					opportunity.getId(), opportunity.getName(),
		...
		for (Opportunity opportunity : opportunityList) {
			...
			if (forecastRecap.getOpportunitiesTypeSelect() == ForecastRecapRepository.OPPORTUNITY_TYPE_BASE) {
				amountCompanyCurr = currencyService
						.getAmountCurrencyConvertedAtDate(opportunity.getCurrency(),
								opportunity.getCompany().getCurrency(),
								opportunity.getAmount().multiply(opportunity.getProbability()).divide(
				...
				amountCompanyCurr = currencyService
						.getAmountCurrencyConvertedAtDate(opportunity.getCurrency(),
								opportunity.getCompany().getCurrency(),
								opportunity.getBestCase().multiply(opportunity.getProbability()).divide(
										new BigDecimal(100), 2, RoundingMode.HALF_UP),
								appBaseService.getTodayDate())
						.setScale(2, RoundingMode.HALF_UP);
			} else {
				amountCompanyCurr = currencyService
						.getAmountCurrencyConvertedAtDate(opportunity.getCurrency(),
								opportunity.getCompany().getCurrency(),
								opportunity.getWorstCase().multiply(opportunity.getProbability()).divide(
										new BigDecimal(100), 2, RoundingMode.HALF_UP),
								appBaseService.getTodayDate())
						.setScale(2, RoundingMode.HALF_UP);
			}
	...
	public void populateWithInvoices(ForecastRecap forecastRecap) throws AxelorException {
		...
		ForecastRecapLineType invoiceForecastRecapLineType = this
				.getForecastRecapLineType(ForecastRecapLineTypeRepository.ELEMENT_INVOICE);
		List<Integer> statusList = StringTool.getIntegerList(invoiceForecastRecapLineType.getStatusSelect());

		if (statusList.isEmpty()) {
			statusList.add(InvoiceRepository.STATUS_VALIDATED);
		}
		invoiceList = invoiceRepo.all().filter("self.company = ?1"
				...
				forecastRecap.getCompany(), forecastRecap.getBankDetails(), statusList,
		...
		for (Invoice invoice : invoiceList) {
			...
			if (invoice.getMove() == null) {
				amount = currencyService
						.getAmountCurrencyConvertedAtDate(invoice.getCurrency(),
								forecastRecap.getCompany().getCurrency(), invoice.getCompanyInTaxTotal(), today)
						.setScale(AppBaseService.DEFAULT_NB_DECIMAL_DIGITS, RoundingMode.HALF_UP);
			} else {
				amount = currencyService.getAmountCurrencyConvertedAtDate(invoice.getCurrency(),
						forecastRecap.getCompany().getCurrency(), invoice.getCompanyInTaxTotalRemaining(), today)
						.setScale(AppBaseService.DEFAULT_NB_DECIMAL_DIGITS, RoundingMode.HALF_UP);
			}
			this.createForecastRecapLine(invoice.getEstimatedPaymentDate(),
					invoiceForecastRecapLineType.getTypeSelect(), amount, invoice.getClass().getName(), invoice.getId(),
					invoice.getInvoiceId(), forecastRecapLineTypeRepo.find(invoiceForecastRecapLineType.getId()),
					forecastRecapRepo.find(forecastRecap.getId()));
			JPA.clear();
		}
	}
	...
	public void populateWithSalaries(ForecastRecap forecastRecap) throws AxelorException {
		...
		ForecastRecapLineType salaryForecastRecapLineType = this
				.getForecastRecapLineType(ForecastRecapLineTypeRepository.ELEMENT_SALARY);
		employeeList = employeeRepo.all().filter(
		...
		while (!itDate.isAfter(forecastRecap.getToDate())) {
			LocalDate payDay = itDate
					.withDayOfMonth(salaryForecastRecapLineType.getPayDaySelect() == 0 ? itDate.lengthOfMonth()
							: salaryForecastRecapLineType.getPayDaySelect());
			if (itDate.isEqual(payDay)) {
				for (Employee employee : employeeList) {
					employeeRepo.find(employee.getId());
					this.createForecastRecapLine(itDate, salaryForecastRecapLineType.getTypeSelect(),
							employee.getMainEmploymentContract().getMonthlyGlobalCost(), employee.getClass().getName(),
	...
	public void populateWithSaleOrders(ForecastRecap forecastRecap) throws AxelorException {
		ForecastRecapLineType saleOrderForecastRecapLineType = this
				.getForecastRecapLineType(ForecastRecapLineTypeRepository.ELEMENT_SALE_ORDER);
		List<Integer> statusList = StringTool.getIntegerList(saleOrderForecastRecapLineType.getStatusSelect());
		...
		if (saleOrderForecastRecapLineType.getEstimatedDuration() != null) {
			saleOrderList = saleOrderRepo.all().filter(
					"(self.expectedRealisationDate BETWEEN ?1 AND ?2 OR (self.creationDate BETWEEN ?3 AND ?4 AND self.expectedRealisationDate IS NULL)) AND self.company = ?5"
							...
							+ (forecastRecap.getBankDetails() != null ? " AND self.companyBankDetails = ?7" : "")
							+ " AND self.timetableList IS EMPTY",
					forecastRecap.getFromDate(), forecastRecap.getToDate(),
					forecastRecap.getFromDate().minusDays(saleOrderForecastRecapLineType.getEstimatedDuration()),
					forecastRecap.getToDate().minusDays(saleOrderForecastRecapLineType.getEstimatedDuration()),
					forecastRecap.getCompany(), statusList, forecastRecap.getBankDetails()).fetch();
		} else {
			saleOrderList = saleOrderRepo
					.all().filter(
							"self.expectedRealisationDate BETWEEN ?1 AND ?2 AND self.company = ?3"
									+ " AND self.statusSelect IN (?4) AND self.inTaxTotal != 0"
									+ (forecastRecap.getBankDetails() != null ? " AND self.companyBankDetails = ?5"
											: "")
									+ " AND self.timetableList IS EMPTY",
							forecastRecap.getFromDate(), forecastRecap.getToDate(), forecastRecap.getCompany(),
							statusList, forecastRecap.getBankDetails())
					.fetch();
		}
		for (SaleOrder saleOrder : saleOrderList) {
			saleOrder = saleOrderRepo.find(saleOrder.getId());
			BigDecimal amount = currencyService
					.getAmountCurrencyConvertedAtDate(saleOrder.getCurrency(), forecastRecap.getCompany().getCurrency(),
							saleOrder.getInTaxTotal(), today)
					.setScale(AppBaseService.DEFAULT_NB_DECIMAL_DIGITS, RoundingMode.HALF_UP);
			...
			this.createForecastRecapLine(
					...
					saleOrderForecastRecapLineType.getTypeSelect(), amount, saleOrder.getClass().getName(),
					saleOrder.getId(), saleOrder.getSaleOrderSeq(),
	...
	public void populateWithPurchaseOrders(ForecastRecap forecastRecap) throws AxelorException {
		ForecastRecapLineType purchaseOrderForecastRecapLineType = this
				.getForecastRecapLineType(ForecastRecapLineTypeRepository.ELEMENT_PURCHASE_ORDER);
		List<Integer> statusList = StringTool.getIntegerList(purchaseOrderForecastRecapLineType.getStatusSelect());
		...
		if (purchaseOrderForecastRecapLineType.getEstimatedDuration() != null) {
			purchaseOrderList = purchaseOrderRepo.all().filter(
					"(self.expectedRealisationDate BETWEEN ?1 AND ?2 OR (self.orderDate BETWEEN ?3 AND ?4 AND self.expectedRealisationDate IS NULL)) AND self.company = ?5"
							...
							+ " AND self.timetableList IS EMPTY",
					forecastRecap.getFromDate(), forecastRecap.getToDate(),
					forecastRecap.getFromDate().minusDays(purchaseOrderForecastRecapLineType.getEstimatedDuration()),
					forecastRecap.getToDate().minusDays(purchaseOrderForecastRecapLineType.getEstimatedDuration()),
					forecastRecap.getCompany(), statusList, forecastRecap.getBankDetails()).fetch();
		} else {
			purchaseOrderList = purchaseOrderRepo
					.all().filter(
							"self.expectedRealisationDate BETWEEN ?1 AND ?2 AND self.company = ?3"
									+ " AND self.statusSelect IN (?4) AND self.inTaxTotal != 0"
									+ (forecastRecap.getBankDetails() != null ? " AND self.companyBankDetails = ?5"
		...
		for (PurchaseOrder purchaseOrder : purchaseOrderList) {
			...
			BigDecimal amount = currencyService
					.getAmountCurrencyConvertedAtDate(purchaseOrder.getCurrency(),
							forecastRecap.getCompany().getCurrency(), purchaseOrder.getInTaxTotal(), today)
					.setScale(AppBaseService.DEFAULT_NB_DECIMAL_DIGITS, RoundingMode.HALF_UP);
			...
			this.createForecastRecapLine(
					...
					purchaseOrderForecastRecapLineType.getTypeSelect(), amount, purchaseOrder.getClass().getName(),
					purchaseOrder.getId(), purchaseOrder.getPurchaseOrderSeq(),
	...
	public void populateWithTimetables(ForecastRecap forecastRecap, ForecastRecapLineType forecastRecapLineType,
			List<Integer> statusList) throws AxelorException {
		...
		if (forecastRecapLineType.getElementSelect() == ForecastRecapLineTypeRepository.ELEMENT_SALE_ORDER) {
			timetableList = timetableRepo.all().filter(
					"self.estimatedDate BETWEEN ?1 AND ?2 AND self.saleOrder.company = ?3"
							+ " AND self.saleOrder.statusSelect IN (?4) AND self.amount != 0"
							+ (forecastRecap.getBankDetails() != null ? " AND self.saleOrder.companyBankDetails = ?5"
									: ""),
					forecastRecap.getFromDate(), forecastRecap.getToDate(), forecastRecap.getCompany(), statusList,
		...
		if (forecastRecapLineType.getElementSelect() == ForecastRecapLineTypeRepository.ELEMENT_SALE_ORDER) {
			for (Timetable timetable : timetableList) {
				...
				BigDecimal amountCompanyCurr = currencyService
						.getAmountCurrencyConvertedAtDate(timetable.getSaleOrder().getCurrency(),
								forecastRecap.getCompany().getCurrency(), timetable.getAmount(), today)
						.setScale(AppBaseService.DEFAULT_NB_DECIMAL_DIGITS, RoundingMode.HALF_UP);
				this.createForecastRecapLine(timetable.getEstimatedDate(), forecastRecapLineType.getTypeSelect(),
						amountCompanyCurr, SaleOrder.class.getName(), timetable.getSaleOrder().getId(),
			...
			for (Timetable timetable : timetableList) {
				timetable = timetableRepo.find(timetable.getId());
				BigDecimal amountCompanyCurr = currencyService
						.getAmountCurrencyConvertedAtDate(timetable.getPurchaseOrder().getCurrency(),
								forecastRecap.getCompany().getCurrency(), timetable.getAmount(), today)
						.setScale(AppBaseService.DEFAULT_NB_DECIMAL_DIGITS, RoundingMode.HALF_UP);
				this.createForecastRecapLine(timetable.getEstimatedDate(), forecastRecapLineType.getTypeSelect(),
						amountCompanyCurr, PurchaseOrder.class.getName(), timetable.getPurchaseOrder().getId(),
	...
	public void getTimetablesOrOrders(ForecastRecap forecastRecap, Map<LocalDate, BigDecimal> mapExpected,
		...
		for (Timetable timetable : timetableSaleOrderList) {
			...
			BigDecimal amountCompanyCurr = currencyService.getAmountCurrencyConvertedAtDate(
					timetable.getSaleOrder().getCurrency(), timetable.getSaleOrder().getCompany().getCurrency(),
					timetable.getAmount(), appBaseService.getTodayDate()).setScale(2, RoundingMode.HALF_UP);
				...
				if (amountCompanyCurr.compareTo(BigDecimal.ZERO) == 0) {
					if (saleOrder.getStatusSelect() == 2) {
						...
						if (mapConfirmed.containsKey(saleOrder.getExpectedRealisationDate())) {
							mapConfirmed.put(saleOrder.getExpectedRealisationDate(),
	...
	public void populateWithForecasts(ForecastRecap forecastRecap) throws AxelorException {
		...
		ForecastRecapLineType forecastForecastRecapLineType = this
				.getForecastRecapLineType(ForecastRecapLineTypeRepository.ELEMENT_FORECAST);
		forecastList = forecastRepo.all()
				.filter("self.estimatedDate < ?1 AND self.company = ?2"
						+ (forecastRecap.getBankDetails() != null ? " AND self.bankDetails = ?3" : "")
						+ " AND self.realizationDate IS NULL", forecastRecap.getToDate(), forecastRecap.getCompany(),
						forecastRecap.getBankDetails())
				.fetch();
		for (Forecast forecast : forecastList) {
			forecast = forecastRepo.find(forecast.getId());
			ForecastReason forecastReason = forecast.getForecastReason();
			LocalDate realizationDate = forecast.getEstimatedDate().isAfter(appBaseService.getTodayDate())
					? forecast.getEstimatedDate()
					: appBaseService.getTodayDate();
			this.createForecastRecapLine(realizationDate, forecast.getAmount().compareTo(BigDecimal.ZERO) == -1 ? 2 : 1,
					forecast.getAmount().abs(), ForecastReason.class.getName(), forecastReason.getId(),
					forecastReason.getReason(), forecastRecapLineTypeRepo.find(forecastForecastRecapLineType.getId()),
					forecastRecapRepo.find(forecastRecap.getId()));
		...
		if (forecastRecap.getBankDetails() != null) {
			forecastList = Beans.get(ForecastRepository.class).all()
					.filter("self.estimatedDate BETWEEN ?1 AND ?2 AND self.company = ?3 AND"
							+ " self.bankDetails = ?4 AND self.realizationDate IS NULL", forecastRecap.getFromDate(),
							forecastRecap.getToDate(), forecastRecap.getCompany(), forecastRecap.getBankDetails())
					.fetch();
		} else {
			forecastList = Beans.get(ForecastRepository.class).all()
					.filter("self.estimatedDate BETWEEN ?1 AND ?2 AND self.company = ?3 AND"
							+ " self.realizationDate IS NULL", forecastRecap.getFromDate(), forecastRecap.getToDate(),
	...
	public void populateWithExpenses(ForecastRecap forecastRecap) throws AxelorException {
		ForecastRecapLineType expenseForecastRecapLineType = this
				.getForecastRecapLineType(ForecastRecapLineTypeRepository.ELEMENT_EXPENSE);
		List<Integer> statusList = StringTool.getIntegerList(expenseForecastRecapLineType.getStatusSelect());

		if (statusList.isEmpty()) {
			statusList.add(ExpenseRepository.STATUS_VALIDATED);
		}
		List<Expense> expenseList = new ArrayList<Expense>();
		expenseList = expenseRepo.all()
				.filter("self.validationDate BETWEEN ?1 AND ?2 AND self.company = ?3 AND self.statusSelect IN (?4)"
		...
		for (Expense expense : expenseList) {
			...
			this.createForecastRecapLine(expense.getValidationDate(), expenseForecastRecapLineType.getTypeSelect(),
					expense.getExTaxTotal(), Expense.class.getName(), expense.getId(), expense.getExpenseSeq(),
	...
	@Transactional
	public void createForecastRecapLine(LocalDate date, int type, BigDecimal amount, String relatedToSelect,
			Long relatedToSelectId, String relatedToSelectName, ForecastRecapLineType forecastRecapLineType,
	...
	public void computeForecastRecapLineBalance(ForecastRecap forecastRecap) {
		...
		Collections.sort(forecastRecapLines, new Comparator<ForecastRecapLine>() {
			...
			public int compare(ForecastRecapLine forecastRecapLine1, ForecastRecapLine forecastRecapLine2) {
				int compareEstimatedDate = forecastRecapLine1.getEstimatedDate()
						.compareTo(forecastRecapLine2.getEstimatedDate());
				int compareTypeSelect = forecastRecapLine1.getForecastRecapLineType().getSequence()
	...
	public String getForecastRecapFileLink(Long forecastRecapId, String reportType) throws AxelorException {
		...
		return ReportFactory
				.createReport("cashManagementForecastRecap", forecastRecapRepo.find(forecastRecapId).getCompany(),
						IReport.FORECAST_RECAP, title + "-${date}")
				.addParam("ForecastRecapId", forecastRecapId.toString())
				.addParam("Locale", ReportSettings.getPrintingLocale(null)).addFormat(reportType).generate()
				.getFileLink();
	}

	protected ForecastRecapLineType getForecastRecapLineType(int elementSelect) throws AxelorException {

		ForecastRecapLineType forecastRecapLineType = forecastRecapLineTypeRepo.all()
				.filter("self.elementSelect = ?1", elementSelect).fetchOne();
		...
		throw new AxelorException(TraceBackRepository.CATEGORY_CONFIGURATION_ERROR,
				I18n.get(IExceptionMessage.FORECAST_RECAP_MISSING_FORECAST_RECAP_LINE_TYPE), elementSelect);

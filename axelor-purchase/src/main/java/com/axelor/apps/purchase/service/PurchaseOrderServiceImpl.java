...
public class PurchaseOrderServiceImpl implements PurchaseOrderService {
	 ...
	 *
	 * <p>
	 * Cette fonction permet de déterminer les tva d'une commande à partir des
	 * lignes de factures passées en paramètres.
	 *
	 * @param purchaseOrder
	...
	public PurchaseOrder createPurchaseOrder(User buyerUser, Company company, Partner contactPartner, Currency currency,
		...
		purchaseOrder
				.setPrintingSettings(Beans.get(TradingNameService.class).getDefaultPrintingSettings(null, company));
	...
	public String getSequence(Company company) throws AxelorException {
		...
		if (seq == null) {
			throw new AxelorException(company, TraceBackRepository.CATEGORY_CONFIGURATION_ERROR,
					I18n.get(IExceptionMessage.PURCHASE_ORDER_1), company.getName());
		}
		return seq;
	}
	...
	public void savePurchaseOrderPDFAsAttachment(PurchaseOrder purchaseOrder) throws AxelorException {
		if (purchaseOrder.getPrintingSettings() == null) {
			throw new AxelorException(TraceBackRepository.CATEGORY_MISSING_FIELD,
					String.format(I18n.get(IExceptionMessage.PURCHASE_ORDER_MISSING_PRINTING_SETTINGS),
		...
		ReportFactory
				.createReport("purchaseOrder", purchaseOrder.getCompany(), IReport.PURCHASE_ORDER, title + "-${date}")
				.addParam("PurchaseOrderId", purchaseOrder.getId().toString()).addParam("Locale", language)
				...
				.toAttach(purchaseOrder).generate().getFileLink();
	}
	...
	public void requestPurchaseOrder(PurchaseOrder purchaseOrder) throws AxelorException {
		...
		Blocking blocking = Beans.get(BlockingService.class).getBlocking(partner, company,
				BlockingRepository.PURCHASE_BLOCKING);
		...
		if (blocking != null) {
			...
			throw new AxelorException(TraceBackRepository.TYPE_FUNCTIONNAL,
					I18n.get(IExceptionMessage.SUPPLIER_BLOCKED) + " " + reason, partner);
		}
	...
	public PurchaseOrder mergePurchaseOrders(List<PurchaseOrder> purchaseOrderList, Currency currency,
			...
			TradingName tradingName) throws AxelorException {
		...
		PurchaseOrder purchaseOrderMerged = this.createPurchaseOrder(AuthUtils.getUser(), company, contactPartner,
				currency, null, numSeq, externalRef, LocalDate.now(), priceList, supplierPartner, tradingName);
	...
	public void updateCostPrice(PurchaseOrder purchaseOrder) throws AxelorException {
		if (purchaseOrder.getPurchaseOrderLineList() != null) {
			for (PurchaseOrderLine purchaseOrderLine : purchaseOrder.getPurchaseOrderLineList()) {
				...
				if (product != null) {
					product.setPurchasePrice(purchaseOrder.getInAti() ? purchaseOrderLine.getInTaxPrice()
							: purchaseOrderLine.getPrice());
					if (product.getDefShipCoefByPartner()) {
						Unit productPurchaseUnit = product.getPurchasesUnit() != null ? product.getPurchasesUnit()
								: product.getUnit();
						BigDecimal convertedQty = Beans.get(UnitConversionService.class).convert(
								purchaseOrderLine.getUnit(), productPurchaseUnit, purchaseOrderLine.getQty(),
								purchaseOrderLine.getQty().scale(), product);
						BigDecimal shippingCoef = Beans.get(ShippingCoefService.class).getShippingCoefDefByPartner(
								product, purchaseOrder.getSupplierPartner(), purchaseOrder.getCompany(), convertedQty);
					...
					if (product.getCostTypeSelect() == ProductRepository.COST_TYPE_LAST_PURCHASE_PRICE) {
						product.setCostPrice(purchaseOrder.getInAti() ? purchaseOrderLine.getInTaxPrice()
								: purchaseOrderLine.getPrice());

...
public class AccountingReportServiceImpl implements AccountingReportService {
	...
	@Inject
	public AccountingReportServiceImpl(AppAccountService appBaseService,
			AccountingReportRepository accountingReportRepo, AccountRepository accountRepo) {
	...
	public String buildQuery(AccountingReport accountingReport) throws AxelorException {
		...
		if (accountingReport.getTypeSelect() == AccountingReportRepository.REPORT_PAYMENT_DIFFERENCES) {
			this.addParams("self.account = ?%d", Beans.get((AccountConfigService.class))
					.getAccountConfig(accountingReport.getCompany()).getCashPositionVariationAccount());
		}

		if (accountingReport.getTypeSelect() == AccountingReportRepository.REPORT_VAT_STATEMENT_INVOICE) {
		...
		this.addParams("(self.move.statusSelect = " + MoveRepository.STATUS_DAYBOOK + " OR self.move.statusSelect = "
	...
	public String getSequence(AccountingReport accountingReport) throws AxelorException {
		...
		if (accountingReportTypeSelect >= 0 && accountingReportTypeSelect < 1000) {
			String seq = sequenceService.getSequenceNumber(SequenceRepository.ACCOUNTING_REPORT,
			...
			String seq = sequenceService.getSequenceNumber(SequenceRepository.MOVE_LINE_EXPORT,
					accountingReport.getCompany());
			if (seq == null) {
				throw new AxelorException(TraceBackRepository.CATEGORY_CONFIGURATION_ERROR,
			...
			String seq = sequenceService.getSequenceNumber(SequenceRepository.ANALYTIC_REPORT,
					accountingReport.getCompany());
			if (seq == null) {
				throw new AxelorException(TraceBackRepository.CATEGORY_CONFIGURATION_ERROR,
	...
	public Account getAccount(AccountingReport accountingReport) {
		if (accountingReport.getTypeSelect() == AccountingReportRepository.REPORT_PAYMENT_DIFFERENCES
			...
			return accountRepo.all().filter("self.company = ?1 AND self.code LIKE '58%'", accountingReport.getCompany())
					.fetchOne();
		}
		return null;
	}
	...
	public BigDecimal getCreditBalance() {
		...
		Query q = JPA.em().createQuery("select SUM(self.credit) FROM MoveLine as self WHERE " + query,
				BigDecimal.class);
	...
	public BigDecimal getDebitBalanceType4() {
		...
		Query q = JPA.em().createQuery("select SUM(self.amountRemaining) FROM MoveLine as self WHERE " + query,
				BigDecimal.class);
	...
	public String getReportFileLink(AccountingReport accountingReport, String name) throws AxelorException {
		return ReportFactory
				.createReport(String.format("accountAccountingReportType%d", accountingReport.getTypeSelect()),
						accountingReport.getCompany(),
				...
				.addFormat(accountingReport.getExportTypeSelect()).toAttach(accountingReport).generate().getFileLink();
	}
	...
	public boolean isThereTooManyLines(AccountingReport accountingReport) throws AxelorException {
		...
		Integer lineMinBeforeLongReportGenerationMessageNumber = accountConfig
				.getLineMinBeforeLongReportGenerationMessageNumber();
		if (lineMinBeforeLongReportGenerationMessageNumber != null
			...
			if (typeSelect > 0 && typeSelect <= AccountingReportRepository.REPORT_GENERAL_LEDGER2) {
				count = Beans.get(MoveLineRepository.class).all().filter(this.getMoveLineList(accountingReport))
						.count();
			} else if (typeSelect == AccountingReportRepository.REPORT_VAT_STATEMENT_RECEIVED) {
				count = Beans.get(TaxPaymentMoveLineRepository.class).all()
						.filter(this.getTaxPaymentMoveLineList(accountingReport)).count();

			} else if (typeSelect == AccountingReportRepository.REPORT_ACQUISITIONS) {
				count = Beans.get(FixedAssetRepository.class).all().filter(this.getFixedAssetList(accountingReport))
						.count();
				count += JPA.em().createQuery(
				...
				count = Beans.get(AnalyticMoveLineRepository.class).all()
						.filter(this.getAnalyticMoveLineList(accountingReport)).count();
			} else {
				return false;
			}
	...
	protected void buildAnalyticMoveLineQuery(AccountingReport accountingReport) {
		...
		this.addParams("(self.moveLine.move.statusSelect = " + MoveRepository.STATUS_DAYBOOK
				+ " OR self.moveLine.move.statusSelect = " + MoveRepository.STATUS_VALIDATED + ")");
	...
	protected void buildFixedAssetQuery(AccountingReport accountingReport) {
		this.initQuery();
		this.addParams("(self.statusSelect = " + FixedAssetRepository.STATUS_VALIDATED + " OR self.statusSelect = "
		...
		if (accountingReport.getTypeSelect() == AccountingReportRepository.REPORT_GROSS_VALUES_AND_DEPRECIATION) {
